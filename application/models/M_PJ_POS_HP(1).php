...
//, "SDKEPALAALKES"
    ...
    $ch = curl_init($url);
      ...
      //  Set Rs3 = xData.BukaRS("select * from zlogemail where LEID = 0", adOpenStatic, adLockOptimistic)
   // zField = Array("LENOIP", "LESTATUS", "LERESPON")
   // zValue = Array(xNoIP, xStatusEmail, left(xRespons, 255))
    //Rs3.AddNew zField, zValue
    
       $result = json_decode($response, true); 
       //{"status":false,"message":"Bad Request","errors":[{"message":"Email tidak valid, silahkan cek email sudah benar"}]}
       $status='';
       $message='';
       $errors=''; 
