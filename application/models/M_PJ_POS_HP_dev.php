...
class M_PJ_POS_HP_dev extends CI_Model {
    ...
    function tambahTransaksi()
    {
        ...
        if ($id_lama!='')
            // jika cancel transksi
            {
                   ...
                   $sql = "update fstokd_tes set sdcancel = 1 where sdidsu  = $id_lama  "; 
                   $query = $this->db->query($sql);  
                  
                   $sql = "update fstoku_tes set  sudp1=0, sutotaldp=0, sustatus = 9 , SUIPBARU = $id  where suid = $id_lama  "; 
                   $query = $this->db->query($sql);   
                  
                   $sql = "delete from fstokd_tes   where sdidsu in (select suid from fstoku_tes where susumber = 'AL' and SUIDUALKES = $id_lama )  "; 
                   $query = $this->db->query($sql);    
                  
                   $sql = "delete from fstoku_tes   where susumber = 'AL' and SUIDUALKES = $id_lama  "; 
                   $query = $this->db->query($sql); 
