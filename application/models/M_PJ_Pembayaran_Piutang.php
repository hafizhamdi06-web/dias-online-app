...
class M_PJ_Pembayaran_Piutang extends CI_Model {
    ...
    function ubahTransaksi(){
        ...
        $sql="CALL SP_JURNAL_PEMBAYARAN_PIUTANG_DEL(".$id.")";
        $this->db->query($sql);

        //Update Header Trans
        $data_header = array(
                        'piusumber' => $this->M_transaksi->prefixtrans(element('PJ_Pembayaran_Piutang',NID)),
                        'piunotransaksi' => $_POST['nomor'],
                        'piutanggal' => tgl_database($_POST['tgl']),
                        'piukontak' => $_POST['kontak'],
                        'piuuraian' => $_POST['uraian'],
                        'piucoakas' => $_POST['coadb'],                
                        ...
                        'piubank' => $_POST['bank'],
                        'piuuang' => $_POST['uangdb'],
                        'piukurs' => $_POST['kursdb'],
        ...
        $sql="CALL SP_JURNAL_PEMBAYARAN_PIUTANG_ADD(".$id.")";
        $this->db->query($sql);

        // USERLOG
        $uactivity = _anomor(element('PJ_Pembayaran_Piutang',NID));
    ...
    function autonumber($tgl){
        $nomor = 0;
        $nomor1 = $this->M_transaksi->prefixtrans(element('PJ_Pembayaran_Piutang',NID));
