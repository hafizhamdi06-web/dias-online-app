...
class PJ_POS_HP extends CI_Controller {
   ...
   function get_item() {
        $query  = "SELECT A.isatuan 'idsatuan', B.snama 'namasatuan', A.iid 'iditem', A.inama 'namaitem', 
                          A.ihargajual1 'hargajual', A.idiskon 'diskon' , A.ikelompok2020 'kelompok2020' , jwajibdokter 'wajibdokter', jbisaeditharga 'bisaeditharga',ISUBKATEGORI 'cetak'
   ...
   function getdata(){
      ...
      $transcode ='IP' ; //$this->M_transaksi->prefixtrans(element('PJ_POS_HP',NID));        
   		$query = "SELECT A.suid 'id', A.sunotransaksi 'nomor', DATE_FORMAT(A.sutanggal,'%d-%m-%Y') 'tanggal',
                       ...
                       F.SDSODID 'medidd_sudahbayar', F.SDPRDID 'medidu_sudahbayar'  
                    FROM fstoku_tes A 
               ...
               LEFT JOIN fstokd_tes J ON A.sudpid=J.sdid     
               LEFT JOIN fstoku_tes K ON J.sdidsu=K.suid 
               ...
               LEFT JOIN fstoku_tes Q ON A.susurgerydpidu=Q.suid 
   ...
   function getdata_dp(){  
   		...
   		$query = " select A.sdid 'id', B.sunotransaksi 'nomor', IFNULL(((A.sdharga-A.sddiskon)*A.sdkeluar)-A.sdbayardp,0) 'nilai'  
                      from fstokd_tes A left join fstoku_tes B on B.suid=A.sdidsu left join bitem C on C.iid = A.sditem
   ...
   function getdata_surgerydp(){  
   		...
   		$query = " select A.suid 'id', A.sunotransaksi 'nomor', A.sutotaltransaksi 'surgerydptotal'  , A.sunilaipiutang 'nilaipiutang'  , A.sutotaltransaksi-A.sunilaipiutang 'surgerydppembayaran',
                      ...
                      from  fstoku_tes A  left join fstokd_tes F on A.suid=F.sdidsu 
                      LEFT JOIN bitem G ON F.sditem=G.iid 
                      LEFT JOIN bsatuan H ON F.sdsatuan=H.sid   
       ...
       $cabang  = @$_SESSION['cabang'] ;
    	...
    	if(empty($_POST['kontak'])) {
   		...
   		$query = " select C.iid 'iditem', C.ikode 'kditem', C.inama 'namaitem', C.ikelompok2020 'item_tipe2020', F.jwajibdokter 'wajibdokter' , 
                       ...
                       ( C.ihargajual1 - (C.ihargajual1*B.pddiskonpersen1/100) - ((C.ihargajual1 - (C.ihargajual1*B.pddiskonpersen1/100))*B.pddiskonpersen2/100)) * B.pdqty  'subtotaldetil' 
                       , A.puid 'idpaket', A.pukode 'kdpaket', A.punama 'namapaket'  , B.pdid 'idpaketdetil'
                       , case when A.pujumlah>1 then coalesce((
                         select sdkedatangan from fstokd_tes 
                         inner join fstoku_tes on suid=sdidsu  
                       ...
                       from fstokd_tes left join fstoku_tes on suid=sdidsu where sdcatatankoli='".$_POST['nopaket']."'  and sdidpotongstok = '".$_POST['idpaket']."'
  ...
  //            Set Rs3 = xData.BukaRS3("SELECT IKELOMPOK2020,emasterpromou.*,IHARGAALKES, IBERLAKU,IID, emasterpromod.*, ISATUAN, IKIDJENISBARU, ICUSTOM9, IKODE,INAMA,SNAMA,ISATUAND,IHARGAJUAL1,IDISKON,IJENISITEM,ITIPEITEM,IKATEGORI,ISTOCKMAKSIMAL,    " & xKolomGudang & " as STOK, IDISKON  FROM emasterpromod inner join emasterpromou on mpuid=mpdidu inner join   bitem on ikode = MPDKELITEM1  inner JOIN bsatuan ON SID=ISATUAN   inner JOIN bitemkelompok ON IKID=IKELOMPOKBARU  WHERE MPDID= " & txtNamaPromo.Tag, adOpenStatic, adLockOptimistic)
       ...
       $cabang  = @$_SESSION['cabang'] ;   
   		...
   		$query = " select MPUJENISPROMO 'jenispromo',
                       ...
                       ( select  count(sditem) as jumlah 
                        from fstokd_tes ZA inner join fstoku_tes ZB on ZB.suid=ZA.sdidsu 
    ...
    function get_detail_pro(){ 
    	if(empty($_POST['medid'])) {
   		...
   		$query = "  select  medCode
   		                ,lmdQuantity 'qtydetil', ihargajual1 'hargadetil', ( lmdPrice- lmdDiscount ) / lmdprice * 100 'dis1detil', 0 'dis2detil', lmdPrice-lmdDiscount 'diskondetil' ,lmdGrandPrice 'subtotaldetil'
    ...
    function get_detail_web_blmiv(){ 
    	if(empty($_POST['webid'])) {
  			...
  			exit;
                ...
                $query = "select  
                      mdcqty 'qtydetil', 
                      case when pasien.ktipe = 12 and (coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)) <> '' or  date_add(coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)), interval 1 year) >= curdate() )then ihargaweb else ihargajual1 end 'hargadetil' 
                    , case when pasien.ktipe = 12 and (coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)) <> '' or  date_add(coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)), interval 1 year) >= curdate() ) then coalesce(IDISKON,0)/100*IHARGAJUAL1 else 0 end 'dis1detil'
                    , 0 'dis2detil' 
                    , case when pasien.ktipe = 12 and (coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)) <> '' or  date_add(coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)), interval 1 year) >= curdate() )  then coalesce(IDISKON,0) else 0 end 'diskondetil'
   ...
   function getdata_bukapassword(){  
        ...
        $password = $_POST['password'] ;
   		...
   		{
   		    $idmenu = 673 ;
            $query  = " select UNAMA,upassword,UID from aausermenu LEFT JOIN aauser ON AUIDUSER=UID where AUIDMENU = '".$idmenu."' and AUAPPROVE = 1 and unama = '".$username."' and upassword = '".$password."' ";
