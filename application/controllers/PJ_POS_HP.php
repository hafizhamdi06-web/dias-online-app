   ...
   function getdata_dp(){  
   		...
   		$query = " select A.sdid 'id', B.sunotransaksi 'nomor', IFNULL(((A.sdharga-A.sddiskon)*A.sdkeluar)-A.sdbayardp,0) 'nilai'  
                      from fstokd A left join fstoku B on B.suid=A.sdidsu left join bitem C on C.iid = A.sditem
                       ...
                       and ((A.sdharga-A.sddiskon)*A.sdkeluar)-A.sdbayardp > 0  and B.sukontak = '".$_POST['xid']."' and sutanggal=current_date 
   ...
   function get_detail_promo(){ 
    ...
    function get_detail_pro(){ 
    	if(empty($_POST['medid'])) {
   		...
   		$query = "  select  medCode
                       ...
                       , iid 'iditem', inama 'namaitem' , ikelompok2020 'item_tipe2020' , H.jwajibdokter 'wajibdokter' , case ijenisitem when 0 then 1 else 0 end 'produk'
                      ...
                      LEFT JOIN bitemjenis H on E.ijenisitem=H.JID  
                       ...
                       WHERE coalesce(lmcDiasIP,'') = '' AND lmcPaymentStatus=0 and  lmdSelected=1 and coalesce(lmdApprovalDokter,1)=1  and medCode in (".$_POST['medid'].")   
    	...
    	if(empty($_POST['webid'])) {
  			...
  			exit;
                ...
                $query = "select  
                    ...
                    , iid 'iditem',inama 'namaitem' , ikelompok2020 'item_tipe2020', H.jwajibdokter 'wajibdokter'  
                    , sid 'idsatuan',skode 'satuan' 
    ...
    function get_detail_web_blmiv(){ 
   	...
   	//	$statusmember = $_POST['statusmember'] ;
                ...
                $query = "select  
                      mdcqty 'qtydetil', 
                      case when pasien.ktipe = 12 and (coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)) <> '' and  date_add(coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)), interval 1 year) >= curdate() )then ihargaweb else ihargajual1 end 'hargadetilx' , ihargajual1 'hargadetil'
                    , case when pasien.ktipe = 12 and (coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)) <> '' and  date_add(coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)), interval 1 year) >= curdate() ) then coalesce(IDISKON,0) else 0 end 'dis1detil'
                    , 0 'dis2detil' 
                    , case when pasien.ktipe = 12 and (coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)) <> '' and  date_add(coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)), interval 1 year) >= curdate() )  then coalesce(IDISKON,0)/100*IHARGAJUAL1 else 0 end 'diskondetil'
                    , case when pasien.ktipe = 12 and (coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)) <> '' and  date_add(coalesce(data_pos_nmw_2023.f_tanggal_akhir (pasien.kid)), interval 1 year) >= curdate() )then ihargaweb*mdcqty else ihargajual1*mdcqty end 'subtotaldetil'
                    , iid 'iditem',inama 'namaitem' , ikelompok2020 'item_tipe2020', H.jwajibdokter 'wajibdokter'  
                    , sid 'idsatuan',skode 'satuan' 
                    , F.kid 'iddokter',F.knama 'namadokter'  
                    , 0 'hargaongkir' 
...
function getnomorip2(){
        ...
        $tgl=$this->input->post('tgl');
        $nomor = 0;
        $nomor1 = 'IP';//$this->M_transaksi->prefixtrans(element('PJ_Penjualan_Tunai',NID));
        $nomor2 = '2501' ; //tgl_notrans($tgl);  

        $notrans_length = strlen($nomor1)+4;

        $sql = "SELECT MAX(RIGHT(sunotransaksi,4)) as 'maks' 
                  FROM fstoku 
                 WHERE MID(sunotransaksi,4,".$notrans_length.")='IP2501' and sucabang='1'  ";
                 
                 $sql = "SELECT 1306 as 'maks'  ";
   ...
   function get_cek_kupon_otc() {
   		if(empty($_POST['notransaksi'])) {
   			echo _pesanError("No Transaksi Kosong !");
  			exit;
   		} 
       
    	if(empty($_POST['tanggal'])) {
   			echo _pesanError("tanggal kosong !");
   			exit;
    	}
       ...
       $cabang  = @$_SESSION['cabang'] ;
   		...
   		$tanggal=tgl_notrans($_POST['tanggal']);
   		$idkontak=$_POST['idkontak'];
               
        $query  = "    select F_SET_JUMLAH_VOUCHER_OTC('".$notransaksi."','".$tanggal."', '".$idkontak."') 'jumlah'   "; 
   ...
   function get_cek_kupon_tindakan() {
   		if(empty($_POST['notransaksi'])) {
   			echo _pesanError("No Transaksi Kosong !");
  			exit;
   		} 
       ...
       $cabang  = @$_SESSION['cabang'] ;
        ...
        $query  = "    select F_SET_JUMLAH_VOUCHER_TINDAKAN('".$notransaksi."','".$tanggal."', '".$idkontak."') 'jumlah'   "; 
   ...
   function cek_apakah_sudah_dapat_kupon() {
        ...
        $query  = " SELECT count(SVKIDVOUCHER) 'jumlah'  FROM fstokkuponv  
        where SVKIDU ='". $idtransaksi ."'
   ...
   function updatekupon3() {
       
   	 
   			echo _pesanError("Save DATA !");
  			exit; 
