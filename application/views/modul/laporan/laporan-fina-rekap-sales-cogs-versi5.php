...
$query .=
	" GROUP BY gkmb.gkode,tmb.sunotransaksi,tmb.sutanggal,kmb.sunotransaksi ";
...
$query .= " GROUP by ctid,npnama,gkode,coalama.ctnama,coabaru.ctnama, ikode, inama, iid, isatuan, icogs
	...
	$query = " SELECT  npnama, gkode, ctid,ctnamabaru,sum(qty) 'qty', sum(cogs) 'cogs'  from (
		SELECT  npnama, gkode,ctid, ctnamabaru, qty, qty*icogs 'cogs' from
                      (
                      select
                      npnama,coabaru.ctid, gkode, coalama.ctnama as ctnamalama, coabaru.ctnama as ctnamabaru, iid, isatuan, ikode, inama, icogs ,
                      IFNULL(SUM(SDMASUK-( IF( SDDARIPAKET<> 0 AND  SDKEDATANGAN  > 0,0,sdkeluar) ) ),0) as qty

		                  from fstokd_stok left join bitem on IID = sditem
                      left join fstoku_stok on suid=sdidsu
...
$query .= " GROUP by npnama,gkode,coalama.ctnama,coabaru.ctnama, ikode, inama, iid, isatuan, icogs
 ...
 ";
 $query .= "  UNION ALL ";
 $query  .=  "SELECT  npnama, gkode, ctid,ctnamabaru,  sum(qty) 'qty', sum(nilaimasuk-nilaikeluar) 'cogs'   from
                       (
                       select
                       npnama, gkode, coalama.ctnama as ctnamalama,coabaru.ctid, coabaru.ctnama as ctnamabaru,
                       sdmasuk-sdkeluar 'qty', icogs,
                       ...
                       left join bitemkelompok2020 on ik2id=ikelompok2020
						 ...
						 sutanggal >= '2026/01/01' and
							sutanggal <'" .	tgl_database($date1) .	"' ";
                        ...
                        group by npnama, gkode, ctnamalama,ctnamabaru, ikode, inama ) zz
                        group by npnama, gkode, ctid,ctnamabaru
                        ";
                        ...
                        $query = "SELECT  npnama, gkode, ctid,ctnamabaru,sum(qty) 'qty', sum(cogs) 'cogs'  from (
	SELECT  ikode,npnama, gkode, ctid,ctnamabaru,sum(qty) 'qty', sum(cogs) 'cogs'  from (
		SELECT  ikode,npnama, gkode,ctid, ctnamabaru, qty, qty*icogs 'cogs' from
                                                                     (
                                                                     select
                                                                     npnama,coabaru.ctid, gkode, coalama.ctnama as ctnamalama, coabaru.ctnama as ctnamabaru, iid, isatuan, ikode, inama, icogs ,
                                                                     IFNULL(SUM(SDMASUK-( IF( SDDARIPAKET<> 0 AND  SDKEDATANGAN  > 0,0,sdkeluar) ) ),0) as qty
		...
		$query .= " AND gpt='" . $idpt . "'";
                                               ...
                                               $query .= " GROUP by npnama,gkode,coalama.ctnama,coabaru.ctnama, ikode, inama, iid, isatuan, icogs
                                                ...
                                                ";
                                                $query .= "  UNION ALL ";
                                                $query  .=  "SELECT  ikode,npnama, gkode, ctid,ctnamabaru,  sum(qty) 'qty', sum(nilaimasuk-nilaikeluar) 'cogs'   from
                                                                      (
                                                                      select
                                                                      npnama, gkode, coalama.ctnama as ctnamalama,coabaru.ctid, coabaru.ctnama as ctnamabaru,
                                                                      sdmasuk-sdkeluar 'qty', icogs,
		                  ...
		                  left join bcoatipe_pendapatan coabaru on coabaru.ctid=i2coapendapatan
		                  WHERE sustatus<>9 and  SDCANCEL=0 and
						 sutanggal between '2026/01/01' and '" .	tgl_database($date1) .	"' ";
							...
							$query .= "
                                                   ...
                                                   group by npnama, gkode, ctnamalama,ctnamabaru, ikode, inama ) zz
                                                   group by npnama, gkode, ctid,ctnamabaru, ikode
                                                   having sum(qty)>0
                                                   ) xx
                                                   group by npnama, gkode, ctid,ctnamabaru
	...
	"SELECT  npnama, gkode, ctnamabaru, ctid,sum(qty) 'qty', sum(qty*icogs) 'cogs' from
	                  ...
	                  AND sutanggal <= '" .tgl_database($date2) ."'";
                       ...
                       group by npnama, gkode, ctid,ctnamabaru
                        ...
                        $query   =  "SELECT  npnama, gkode, ctid,ctnamabaru,sum(qty) 'qty', sum(cogs) 'cogs'  from (
                        SELECT  ikode,npnama, gkode, ctid,ctnamabaru,  sum(qty) 'qty', sum(nilaimasuk-nilaikeluar) 'cogs'   from
                                           (
                                           select
                                           npnama, gkode, coalama.ctnama as ctnamalama,coabaru.ctid, coabaru.ctnama as ctnamabaru,
                                           sdmasuk-sdkeluar 'qty', icogs,
		                  ...
		                  left join bcoatipe_pendapatan coabaru on coabaru.ctid=i2coapendapatan
		                  WHERE sustatus<>9 and  SDCANCEL=0 and
						 sutanggal between '2026/01/01' and '" .	tgl_database($date2) .	"' ";
	...
	<table class="table">
		...
		<tbody>
			<?
				...
				$sakenergy=0;$sakmedical=0;$sakiv=0;$sakskincare=0;$saklab=0;
...
$query =
	...
	SELECT  case when a.susumber = 'IP' then 'POS'
				...
				when a.susumber = 'SJ' and gkmb.gpt<>b.gpt then 'Penjualan Beda PT'
				when a.susumber = 'SJ' and gkmb.gpt=b.gpt then 'Penjualan Sama PT'
		...
		<tbody>
			<?
				...
				foreach ($datareport->data as $row) {
                        ...
                        elseif($row->jenis=='Penjualan Sama PT')
                        {$nilaisjspt=$row->nilaimasuk-$row->nilaikeluar;}
                        elseif($row->jenis=='Penjualan Beda PT')
                        {$nilaisjbpt=$row->nilaimasuk-$row->nilaikeluar;}
...
$query .=
	" GROUP by npnama,gkode,  coalama.ctnama ,  tindakan.sunotransaksi  ,  b.ikode  ";
	...
	<table class="table" style="width:50%;">
    	...
    	2,
			...
			<tr>
				...
				<td class="right px-1"><?= $tnilaisalesip > 0
    	? eFormatNumber(($totalcogs / $tnilaisalesip) * 100, 2)
    	: 0 ?> % </td>
			</tr>
			<tr class="bg-dark">
    	...
    	2,
			...
			<tr>
				<td class="left px-1">Penjualan SJ Beda PT</td>
				<td class="right px-1"><?= eFormatNumber($nilaisjbpt, 2) ?></td>
			</tr>
			<tr>
				<td class="left px-1">Penjualan SJ Sama PT</td>
				<td class="right px-1"><?= eFormatNumber($nilaisjspt, 2) ?></td>
			</tr>
		 ...
		 <? $nilaistoksiapdijual=$totalnilaiawal+$nilaipb+$nilaitmbbpt+$nilaitmbspt+$nilaikmbbpt+$nilaikmbspt+$nilairpb+$nilailain+$nilaipbs+$nilaisjbpt+$nilaisjspt;
           ?>
			<tr>
