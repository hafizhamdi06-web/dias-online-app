<?php
    ...
    $CI =& get_instance();  
	                  ...
	                  $query = "SELECT  npnama, gkode, ctnamalama,ctnamabaru, ikode, inama, qty, nilaimasuk-nilaikeluar as nilai from
                      (
                      select
                      npnama, gkode, coalama.ctnama as ctnamalama, coabaru.ctnama as ctnamabaru,  ikode, inama,  
					  IFNULL(sum(case when sdmasuk>0 then sdmasuk*sdharga else 0 end),0) as nilaimasuk,
					  IFNULL(sum(case when sdkeluar>0 then IF(SDDARIPAKET<> 0 AND  SDKEDATANGAN  = 0,0,sdbeli) else 0 end),0) as nilaikeluar, 
	                  ...
	                  AND sutanggal between '2026/01/01' and  '".tgl_database($date)."'  
                      ";     
	                   ...
	                   if($idpt != ""){
                        ...
                        $query .= " GROUP by npnama,gkode,coalama.ctnama,coabaru.ctnama, ikode, inama
	...
	<table class="table">
		<thead> 
			<tr class="bg-dark"> 
				...
				<th class="left px-1">COGS Rata Rata</th>  
			</tr>
		</thead>
		<tbody>
			<?	
				...
				foreach ($datareport->data as $row) {
    			   ...
    			   if ($namapt !=  $row->npnama and $namapt !='') {
    			    ...
    			    $namapt = $row->npnama ;  
    					...
    					echo "<td class='right px-1'>".eFormatNumber($row->nilai/$row->qty,2)."</td>"; 
    					echo "</tr>";	
    					 
    					$tnilaiproduk += $row->nilai ; 
    					$nilaiprodukperpt += $row->nilai ; 
