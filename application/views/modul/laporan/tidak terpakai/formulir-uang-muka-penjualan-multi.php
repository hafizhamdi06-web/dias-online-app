<?php
    ...
    $query  = "SELECT A.dpid 'id',A.dpnotransaksi 'nomor',DATE_FORMAT(A.dptanggal,'%d-%m-%Y') 'tanggal',B.knama 'kontak',
            ...
            LEFT JOIN bcoa D ON A.dpcdp = D.cid";

    if(empty($this->input->post('nomorsampai')) && !empty($this->input->post('nomor'))){
	    $query .= " WHERE A.dpnotransaksi = '".$this->input->post('nomor')."'";    	
    }
    if(!empty($this->input->post('nomorsampai')) && !empty($this->input->post('nomor'))){
	    $query .= " WHERE A.dpnotransaksi BETWEEN '".$this->input->post('nomor')."' AND '".$this->input->post('nomorsampai')."'";    	
    }
