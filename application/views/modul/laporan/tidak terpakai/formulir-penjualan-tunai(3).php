<?php
    ...
    $querydtl  = "SELECT B.ikode 'noitem',B.inama 'item',A.sdkeluar 'qty',C.skode 'satuan',A.sdcatatan 'catatan', concat(sdnoref,'-',sdlantai2,'-',left(E.knama,6),'-',left(F.knama,6)) 'ket',
                        A.sdharga 'harga', A.sddiskonpersen 'dis1', A.sddiskonpersen2 'dis2', A.sddiskon 'diskon', A.sdkeluar * (A.sdharga-A.sddiskon) 'subtotal',
                        coalesce(H.pukode,'')  'kodepaket' , J.mpukode 'kodepromo' , H.pucetakheadersaja 'cetakheadersaja' 
...
<div class="header-report">
    	...
    	<table class="table" style="margin-top: 0px;">
			<tbody>
				<tr>
					<td width="10%"><h3 class="text-blue"><b>NMW</b></h3></td>
					<td ><span>IG : nmwskincare www.nmwskincare.co.id Telp : 021-777 7777</td> 
	...
	<table class="table">
		<thead>
			<tr class="bg-none"> 			
				...
				<th align="center" width="5%" >Ref-IC-Dokt-Perawat</th>	 
		...
		<tbody>
			<?
				$i = 1;
				$qty = 0; $namapaket =''; $namapromo=''; $sumnilaipaket=0;
			    foreach ($detil->data as $row) {
    			  ...
    			  // dan jika cetak header saja, maka detailnya tidak ditampilkan
    				...
    				<tr> 
    					...
    					<td class="right px-1 py-0"><?= eFormatNumber($row->qty,0) ?></td>
    			    ...
    			    <?    
    			     ...
    			     if ($row->kodepaket!='') {
    			          if ($namapaket!='') {
    			            if ($row->kodepaket <>   $namapaket ) { 
                			    ?>
                				<tr> 
                					<td class="px-1 py-0">Paket <?= $row->kodepaket ?></td> 
                					<td class="right px-1 py-0"></td>
                					<td class="right px-1 py-0"></td>
                					<td class="right px-1 py-0"></td>
                					<td class="right px-1 py-0"></td>
                					<td class="right px-1 py-0"></td>
	...
	<table class="table" >
		<tbody> 
        			<tr>
        				...
        				<td width="15%"  class="px-1 py-0">Kredit</td> 
        			...
        			<tr>
        				...
        				<td class="px-1 py-0">Transfer</td> 
        			...
        			<tr>
        				...
        				<td class="px-1 py-0">Merchant</td> 
        			...
        			<?
        			    ...
        			    { ?> 
        			        	<tr>
                    				<td class="px-1 py-1">No Kartu Kredit</td> 	
                    				<td class="px-1 py-1"><?= $kreditno ?> </td> 
                    				<td class="px-1 py-1">Jenis</td> 	
                    				<td class="px-1 py-1"><?= $kreditjenis ?> </td>  
                    			</tr>  
        			     <?    }
        			    ...
        			    { ?> 
        			        	<tr>
                    				<td class="px-1 py-1">No Kartu Debit</td> 	
                    				<td class="px-1 py-1"><?= $debitno ?> </td> 
                    				<td class="px-1 py-1">Jenis</td> 	
                    				<td class="px-1 py-1"><?= $debitjenis ?> </td>  
                    			</tr>  
        			     <?    } 
        			?>
        			
        				        <tr>
                    				<td colspan=6 align="center"  class="px-1 py-3">Sudah termasuk jasa dokter dan ppn 11% atas produk</td>  
                    			</tr>
        				        <tr>
