...
<?
                    ...
                    $query2 = " SELECT  SUCABANG,   F_SUBTOTAL_MASKER(SUID) AS masker,SUID,SDKARYAWAN, F_SUBTOTAL_SPA(SUID) as totalspa, F_ADARESEP(suid) as adaresep, 
                                F_ADARESEP_RACIKANAPOTIK(suid) as racikanapotik,coalesce(sdreferal,0) as referal, iresep,iresepitter,
                                ik2kode,ikomisi2020 as ikelompok2020,IMODEL, 
                                coalesce(F_TOTAL_ALKES_BY_URUTAN(suid,case when sdurutanawal=0 then sdurutan else sdurutanawal end),0)  as alkes,sdkeluar,sdharga,
                                sddiskon,sunotransaksi,sutanggal,ikode,ikelompok21, pelanggan.KNAMA AS NAMApelanggan,sales.KNAMA AS namasales,sales.KKODE AS kodesales  
                                ,  sales.KIDABSENSI AS IDABSENSI, sales.KIDEMPLOYEE  AS IDEMPLOYEE ,  sales.KNAMAEMPLOYEE AS NAMAEMPLOYEE,  referal.kkode as kodereferal,
                                epaketu.* , npnamaclinic, sdkeluar*(sdharga-sddiskon) as subtotal  
                                ...
                                left join bgudang on gid=sucabang left join bnamapt on npid=gpt  WHERE    SUSTATUS<>9 and SUSUMBER = 'IP' 
...
?>
	...
	<table class="table">
		<thead>
			<tr class="bg-dark">
				<th class="left px-1" colspan="5">Komisi Perawat Therapist</th>   
			</tr>
			<tr class="bg-dark">
				...
				foreach ($datareport2->data as $row) { 
    			    ...
    			    $subtotalmedical += $row->subtotal; 
    			        ...
    			        if ($ikelompok2020==3) {  
                			      ...
                			      if ( round($totalspa,1) >= 150000 and $noip != $row->sunotransaksi ) {
                			          $qty_spa_perhari++ ;
    			   ...
    			   if ($ikelompok2020==1) {
    			    ...
    			    $gkomisi += $komisi ;
    					...
    					echo "<td class='px-1'>".$row->gkode."</td>";  
    					echo "<td class='px-1'>".$row->namasales."</td>"; 
    					echo "<td class='px-1'>".$ikelompok2020."</td>"; 
