...
var _transaksidatatable = function(_view=''){
  ...
  tabeltransaksi=$('#transaksi-table').DataTable({
    ...
    "ajax": {
        "url":base_url+"Datatable_Master/view_table_paket",
        "type":"post",
        "beforeSend": function(){
...
function restable(){
  const id = $('#transaksi-table').DataTable().cell($('#transaksi-table').DataTable().rows({selected:true}),0).data(),
        jumlah = $('#transaksi-table').DataTable().cell($('#transaksi-table').DataTable().rows({selected:true}),4).data(),
      trigger = $('#modaltrigger').val();         
  $('#modal').modal('hide');        
  $("#"+trigger).contents().find("#jumlahpaket").val(jumlah).trigger('change');
  $("#"+trigger).contents().find("#idpaket").val(id).trigger('change');
  return;        
}