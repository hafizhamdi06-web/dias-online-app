...
var _kontakdatatable = function(_view='' ){
  katId = $('#idu').val(); 
  tabelkontak= $('#contact-table').DataTable({
    ...
    "ajax": {
        ...
        "url":base_url+"Datatable_Master/"+_view+"/"+katId,
        "type":"post",
        "beforeSend": function(){
...
function _tampilkankupon(jumlahkupon){
   alert('Menampilkan NO Voucher');
...
var _saveData = (function(){ 
  ...
  const idu = $("#idu").val() ; 
  const jumlahkupon = $("#jumlahkupon").val() ;
  ...
  if(jumlahkupon!=totalcek) {
    alert("Jumlah Kupon terpilih salah") ;   
    return ;
  ...
  rey.set('id',idu);        
  rey.set('jumlahkupon',jumlahkupon);             
  rey.set('detil',id);
