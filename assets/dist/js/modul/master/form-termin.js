...
var _IsValid = (function(){
    if ($('#kode').val()==''){
      $('#kode').attr('data-title','Kode termin harus diisi !');      
      $('#kode').tooltip('show');
      $('#kode').focus();
      return 0;
    }
    if ($('#nama').val()==''){
      $('#nama').attr('data-title','Nama termin harus diisi !');      
      $('#nama').tooltip('show');
      $('#nama').focus();
      return 0;
    }
    if ($('#tempo').val()==''){
      $('#tempo').attr('data-title','Tempo harus diisi !');      
      $('#tempo').tooltip('show');
      $('#tempo').focus();
      return 0;
    }    
...
var _saveData = (function(){
  ...
  $.ajax({ 
    "url"    : base_url+"Master_Termin/savedata", 
    "type"   : "POST", 
    "data"   : rey,
    "processData": false,
    "contentType": false,
    ...
    "success": function(result) {
      ...
      if(result=='sukses'){
        $('#modal').modal('hide');                
        toastr.success("Data termin berhasil disimpan");                  
        return;
      } else {        
        toastr.error(result);                          
        return;
...
function _getData(id){
    ...
    $.ajax({ 
      "url"    : base_url+"Master_Termin/getdata",       
      "type"   : "POST", 
      "dataType" : "json", 
      "data" : "id="+id,
      "cache"  : false,
