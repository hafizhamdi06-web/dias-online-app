...
var _IsValid = (function(){
    if ($('#kode').val()==''){
      $('#kode').attr('data-title','Kode satuan harus diisi !');      
      $('#kode').tooltip('show');
      $('#kode').focus();
      return 0;
    }
    if ($('#nama').val()==''){
      $('#nama').attr('data-title','Nama satuan harus diisi !');      
      $('#nama').tooltip('show');
      $('#nama').focus();
      return 0;
    }
...
var _saveData = (function(){
  ...
  var sdasar = 1;
  if ($('#chkdasar').is(":checked")) sdasar = 0;
  ...
  rey.set('nama',nama);
  rey.set('nilai',nilai);  
  rey.set('sdasar',sdasar);    

  $.ajax({ 
    "url"    : base_url+"Master_Satuan/savedata", 
    "type"   : "POST", 
    "data"   : rey,
    "processData": false,
    "contentType": false,
    ...
    "success": function(result) {
      ...
      if(result=='sukses'){
        $('#modal').modal('hide');                
        toastr.success("Data satuan berhasil disimpan");                  
        return;
      } else {        
        toastr.error(result);                          
        return;
...
function _getData(id){
    ...
    $.ajax({ 
      "url"    : base_url+"Master_Satuan/getdata",       
      "type"   : "POST", 
      "dataType" : "json", 
      "data" : "id="+id,
      "cache"  : false,
      ...
      "success" : function(result) {
        if (typeof result.pesan !== 'undefined') { // Jika ada pesan maka tampilkan pesan
          ...
          if(result.data[0]['sdasar']=='0'){
            $("#chkdasar").prop('checked',true);
          }
