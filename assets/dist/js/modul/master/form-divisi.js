...
var _IsValid = (function(){
    if ($('#kode').val()==''){
      $('#kode').attr('data-title','Kode divisi harus diisi !');      
      $('#kode').tooltip('show');
      $('#kode').focus();
      return 0;
    }
    if ($('#nama').val()==''){
      $('#nama').attr('data-title','Nama divisi harus diisi !');      
      $('#nama').tooltip('show');
      $('#nama').focus();
      return 0;
    }
...
var _saveData = (function(){
  ...
  $.ajax({ 
    "url"    : base_url+"Master_Divisi/savedata", 
    "type"   : "POST", 
    "data"   : rey,
    "processData": false,
    "contentType": false,
    ...
    "success": function(result) {
      ...
      if(result=='sukses'){
        $('#modal').modal('hide');                
        toastr.success("Data divisi berhasil disimpan");                  
        return;
      } else {        
        toastr.error(result);                          
        return;
...
function _getData(id){
    ...
    $.ajax({ 
      "url"    : base_url+"Master_Divisi/getdata",       
      "type"   : "POST", 
      "dataType" : "json", 
      "data" : "id="+id,
      "cache"  : false,
