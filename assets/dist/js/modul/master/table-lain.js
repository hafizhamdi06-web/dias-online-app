...
$(function () {
  ...
  Component_Select2('#tipe',`${base_url}Select_Master/view_jenis_lain`,'form_tipe','Tipe');
  ...
  tabel=$('#lain-table').DataTable({
    ...
    "dom": '<"top"pi>tr<"clear">',
    "ajax": {
        "url":base_url+"Datatable_Master/view_table_lain",
        "type":"post",
        "data": function(data){
          data.kode = $('#kode').val();
          data.nama = $('#nama').val();
...
$("#badd").click(function() {
  $.ajax({
    "url"    : base_url+"Modal/form_lain",
    "type"   : "POST",
    "dataType" : "html",
    "beforeSend": function(){
      parent.window.$(".loader-wrap").removeClass("d-none");
      parent.window.$(".modal").modal("show");
      parent.window.$(".modal-title").html("Tambah Data Lain");
      parent.window.$("#modaltrigger").val("iframe-page-lain");
    },
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");
      console.log('error menampilkan form data lain...');
      return;
    },
...
$("#bdelete").click(function() {
  const id = $('#lain-table').DataTable().cell($('#lain-table').DataTable().rows({selected:true}),2).data();
...
$("#bedit").click(function() {
  const id = $('#lain-table').DataTable().cell($('#lain-table').DataTable().rows({selected:true}),0).data();
  ...
  $.ajax({
    ...
    "beforeSend": function(){
      parent.window.$(".loader-wrap").removeClass("d-none");
      parent.window.$(".modal").modal("show");
      parent.window.$(".modal-title").html("Data Lain");
      parent.window.$("#modaltrigger").val("iframe-page-lain");
    },
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");
...
$('#lain-table').on('dblclick','tr',function(e){
    e.preventDefault();
    e.stopPropagation();
...
var _deleteData = (function(){
  ...
  $.ajax({
    "url"    : base_url+"Master_Lain/deletedata",
    "type"   : "POST",
    "data"   : "id="+id,
    "cache"    : false,
    "beforeSend" : function(){
...
$("#bfilter").click(function() {
  if($("#fDataTable").hasClass("d-none")){
    $("#lain-table").removeClass("w-100");
    $("#lain-table").addClass("w-75");
    $("#fDataTable").removeClass("d-none");
        $(".noresultfound-x").css("background-position","30% 160px");
  }else {
    $("#lain-table").removeClass("w-75");
    $("#lain-table").addClass("w-100");
...
$("#submitfilter").click(function() {
  $('#lain-table').DataTable().ajax.reload();
