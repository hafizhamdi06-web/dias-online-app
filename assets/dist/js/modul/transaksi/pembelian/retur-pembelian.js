...
/* ========================================================================================== */
  ...
  $("#bTable").click(function() {
    ...
    location.href=base_url+"page/rpbData";      
  });
  ...
  $("#bViewJurnal").click(function() {
      ...
      $.ajax({ 
        ...
        "beforeSend": function(){
          ...
          parent.window.$("#modaltrigger").val("iframe-page-rpb");
  ...
  $("#bprint").click(() => {
      if($('#id').val()=='') return;    
      window.open(`${base_url}Laporan/preview/page-rpb/${$("#id").val()}`)    
  });
  ...
  $("#bsearch").click(function() {    
    $.ajax({ 
      ...
      "success": function(result) {
        parent.window.$(".main-modal-body").html(result);    
        parent.window.$('.modal-body').css('min-height','calc(100vh - 30vh)');        
        parent.window._transaksidatatable('view_retur_pembelian');
    		setTimeout(function (){
    		   parent.window.$('#modal input').focus();
    		}, 500);
        return;
  ...
  $(this).on("select2:select", "select[name^='item']", function(){
      ...
      $.ajax({ 
        "url"    : base_url+"PB_Retur_Pembelian/get_item", 
        "type"   : "POST", 
        "data"   : "id="+$(this).val(),
        "dataType" : "json", 
        "cache"  : false,
...
var _addRow = () => {
  let newrow = " <tr>";
      ...
      newrow += "<td><input type=\"hidden\" name=\"noref[]\" class=\"noref\"><textarea name=\"catatan[]\" class=\"form-control form-control-sm\" rows=\"1\" autocomplete=\"off\" style=\"height:2em;resize:none\"></textarea></td>";
...
var _deleteData = () => {
  ...
  $.ajax({ 
    "url"    : base_url+"PB_Retur_Pembelian/deletedata", 
    "type"   : "POST", 
...
var _saveData = () => {
  ...
  $.ajax({ 
    "url"    : base_url+"PB_Retur_Pembelian/savedata", 
    "type"   : "POST", 
    "data"   : rey,
    "processData": false,
    "contentType": false,
      ...
      if(result.pesan=='sukses'){
          parent.window.Swal.fire({
              ...
              if (printing.isConfirmed) {
                window.open(`${base_url}Laporan/preview/page-rpb/${result.nomor}`)
              }
...
var _getDataTransaksi = (id) => {
  ...
  $.ajax({ 
    "url"    : base_url+"PB_Retur_Pembelian/getdata",       
    "type"   : "POST", 
    "dataType" : "json", 
    "data" : "id="+id,
    "cache"  : false,
    ...
    "error"  : function(){
      parent.window.toastr.error('Error : Gagal mengambil data transaksi retur pembelian !');
      parent.window.$('.loader-wrap').addClass('d-none');                  
      return;
    },
    "success" : function(result) {
