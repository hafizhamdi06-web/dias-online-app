...
/* ========================================================================================== */
  ...
  $("#bTable").click(function() {
    ...
    location.href=base_url+"page/btrData";      
  });
  ...
  $("#bViewJurnal").click(function() {
      ...
      $.ajax({ 
        ...
        "beforeSend": function(){
          ...
          parent.window.$("#modaltrigger").val("iframe-page-btr");
    ...
    if($(this).attr('role')) {        
      ...
      $.ajax({ 
        ...
        "beforeSend": function(){
          parent.window.$('.loader-wrap').removeClass('d-none');          
          parent.window.$(".modal").modal("show");                  
          parent.window.$(".modal-title").html("Cari Surat Jalan");
          parent.window.$("#modaltrigger").val("iframe-page-btr");        
        },       
        "error": function(){
          parent.window.$('.loader-wrap').addClass('d-none');          
          console.log('error menampilkan modal cari surat jalan...');
          return;
        },
        "success": function(result) {
          parent.window.$(".main-modal-body").html(result);      
          parent.window.$('.modal-body').css('min-height','calc(100vh - 30vh)');        
          parent.window._transaksidatatable('view_cari_surat_jalan_r',$('#idkontak').val());
          setTimeout(function (){
               parent.window.$('#modal input').focus();
          }, 500);
          return;
  ...
  $("#bsearch").click(function() {    
    $.ajax({ 
      ...
      "success": function(result) {
        parent.window.$(".main-modal-body").html(result);    
        parent.window.$('.modal-body').css('min-height','calc(100vh - 30vh)');          
        parent.window._transaksidatatable('view_terima_retur');
        setTimeout(function (){
             parent.window.$('#modal input').focus();
        }, 500);
        return;
  ...
  $("#bprint").click(() => {
      if($('#id').val()=='') return;    
      window.open(`${base_url}Laporan/preview/page-btr/${$("#id").val()}`)    
  });
  ...
  $(this).on("select2:select", "select[name^='item']", function(){
      ...
      $.ajax({ 
        "url"    : base_url+"PJ_Terima_Retur/get_item", 
        "type"   : "POST", 
        "data"   : "id="+$(this).val(),
        "dataType" : "json", 
        "cache"  : false,
...
var _deleteData = () => {
  ...
  $.ajax({ 
    "url"    : base_url+"PJ_Terima_Retur/deletedata", 
    "type"   : "POST", 
...
var _saveData = () => {
  ...
  $.ajax({ 
    "url"    : base_url+"PJ_Terima_Retur/savedata", 
    "type"   : "POST", 
    "data"   : rey,
    "processData": false,
    "contentType": false,
      ...
      if(result.pesan=='sukses'){
          parent.window.Swal.fire({
              ...
              if (printing.isConfirmed) {
                window.open(`${base_url}Laporan/preview/page-btr/${result.nomor}`)
              }
...
var _getDataTransaksi = (id) => {
    ...
    $.ajax({ 
      "url"    : base_url+"PJ_Terima_Retur/getdata",       
      "type"   : "POST", 
      "dataType" : "json", 
      "data" : "id="+id,
      "cache"  : false,
      ...
      "error"  : function(){
        alert('Error : Gagal mengambil data transaksi penerimaan barang retur !');
        parent.window.$('.loader-wrap').addClass('d-none');                  
        return;
      },
      "success" : function(result) {
...
var _getDataReferensi = (id) => {
  if(id=='' || id==null) return;  

  $.ajax({ 
    "url"    : base_url+"PJ_Terima_Retur/getdatasuratjalan",       
    "type"   : "POST", 
    "dataType" : "json", 
    "data" : "id="+id.toString(),
    "cache"  : false,
    ...
    "error"  : function(xhr){
      console.log(xhr.responseText);
      alert('Error : Gagal mengambil data surat jalan !');
      parent.window.$('.loader-wrap').addClass('d-none');                  
      return;
    },
