  ...
  $("#bViewJurnal").click(function() {
      ...
      $.ajax({ 
        ...
        "beforeSend": function(){
          ...
          parent.window.$("#modaltrigger").val("iframe-page-opj");
  ...
  $("#bsearch").click(function() {    
    $.ajax({ 
      ...
      "success": function(result) {
        parent.window.$(".main-modal-body").html(result);  
        parent.window.$('.modal-body').css('min-height','calc(100vh - 30vh)');            
        parent.window._transaksidatatable('view_order_penjualan');
          setTimeout(function (){
               parent.window.$('#modal input').focus();
          }, 500);
        return;
...
var _deleteData = () => {
  ...
  $.ajax({ 
    "url"    : base_url+"PJ_Order_Penjualan/deletedata", 
    "type"   : "POST", 
...
var _saveData = () => {
  ...
  rey.set('alamatkirim',alamatkirim);   
  ...
  $.ajax({ 
    "url"    : base_url+"PJ_Order_Penjualan/savedata", 
    "type"   : "POST", 
    "data"   : rey,
    "processData": false,
    "contentType": false,
...
var _getDataTransaksi = (id) => {
  ...
  $.ajax({ 
    "url"    : base_url+"PJ_Order_Penjualan/getdata",       
    "type"   : "POST", 
    "dataType" : "json", 
    "data" : "id="+id,
    "cache"  : false,
    ...
    "error"  : function(){
      alert('Error : Gagal mengambil data transaksi order penjualan !');
      parent.window.$('.loader-wrap').addClass('d-none');                  
      return;
    },
    "success" : function(result) {
