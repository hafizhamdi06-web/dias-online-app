...
/* ========================================================================================== */
  ...
  $("#bTable").click(function() {
    ...
    location.href=base_url+"page/fpjData";      
  });  
...
//    alert(stringpb);
    $.ajax({ 
      ...
      "beforeSend": function(){
        parent.window.$('.loader-wrap').removeClass('d-none');          
        parent.window.$(".modal").modal("show");                  
        parent.window.$(".modal-title").html("Cari Pengiriman Barang");
        parent.window.$("#modaltrigger").val("iframe-page-fpj");        
      },
      "error": function(){
        parent.window.$('.loader-wrap').addClass('d-none');          
        console.log('error menampilkan modal cari pengiriman barang...');
        return;
      },
      "success": function(result) {
        parent.window.$(".main-modal-body").html(result);   
        ...
        parent.window._transaksidatatable('view_cari_surat_jalan_new',$('#idkontak').val());
        setTimeout(function (){
             parent.window.$('#modal input').focus();
        }, 500);
        return;
  ...
  $("#bprint").click(() => {
      if($('#id').val()=='') return;    
      window.open(`${base_url}Laporan/preview/page-fpj/${$("#id").val()}`)    
  });
  ...
  $("#buangmuka").click(function() {    
    ...
    $.ajax({ 
      ...
      "beforeSend": function(){
        parent.window.$('.loader-wrap').removeClass('d-none');          
        parent.window.$(".modal").modal("show");                  
        parent.window.$(".modal-title").html("Cari Uang Muka Penjualan");
        parent.window.$("#modaltrigger").val("iframe-page-fpj");
        parent.window.$('#coltrigger').val('iddp');                
      },
      "error": function(){
        parent.window.$('.loader-wrap').addClass('d-none');          
        console.log('error menampilkan modal cari uang muka penjualan...');
        return;
      },
      "success": function(result) {
        parent.window.$(".main-modal-body").html(result);    
        ...
        parent.window._transaksidatatable('view_cari_uang_muka_penjualan',$('#idkontak').val());
        setTimeout(function (){
  ...
  $("#bsearch").click(function() {    
    $.ajax({ 
      ...
      "success": function(result) {
        parent.window.$(".main-modal-body").html(result);     
        parent.window.$('.modal-body').css('min-height','calc(100vh - 30vh)');         
        parent.window._transaksidatatable('view_faktur_penjualan');
        setTimeout(function (){
             parent.window.$('#modal input').focus();
        }, 500);
        return;
  ...
  $(this).on("select2:select", "select[name^='item']", function(){
      ...
      $.ajax({ 
        "url"    : base_url+"PJ_Faktur_Penjualan/get_item", 
        "type"   : "POST", 
...
var _IsValid = () => {
    ...
    for(let i=0;i<totalbaris;i++){
      ...
      if($("input[name^='sunotrans']").eq(i).val()==''){
        parent.window.Swal.fire({
            title: `No. Pengiriman pada baris ke-${i+1} harus diisi !`,
            showDenyButton: false,
            showCancelButton: false,
            confirmButtonText: `Tutup`,
        }).then((result) => {
...
var _saveData = () => {
  ...
  $.ajax({ 
    "url"    : base_url+"PJ_Faktur_Penjualan/savedata", 
    "type"   : "POST", 
    "data"   : rey,
    "processData": false,
    "contentType": false,
      ...
      if(result.pesan=='sukses'){
          parent.window.Swal.fire({
              ...
              if (printing.isConfirmed) {
                window.open(`${base_url}Laporan/preview/page-fpj/${result.nomor}`)
              }
...
var _getDataTransaksi = (id) => {
  ...
  $.ajax({ 
    "url"    : base_url+"PJ_Faktur_Penjualan/getdata",       
    "type"   : "POST", 
    "dataType" : "json", 
    "data" : "id="+id,
    "cache"  : false,
    ...
    "error"  : function(){
      parent.window.toastr.error('Error : Gagal mengambil data transaksi faktur penjualan !');
      parent.window.$('.loader-wrap').addClass('d-none');                  
      return;
    },
    "success" : function(result) {
...
var _getDataBKG = (id) => {
  ...
  $.ajax({ 
    "url"    : base_url+"PJ_Faktur_Penjualan/getdatapengiriman",       
    "type"   : "POST", 
    "dataType" : "json", 
    "data" : "id="+id.toString(),
    "cache"  : false,
    ...
    "error"  : function(xhr){
      console.log(xhr.responseText);
      alert('Error : Gagal mengambil data pengiriman barang !');
      parent.window.$('.loader-wrap').addClass('d-none');                  
      return;
    },
    "success" : async function(result){
...
var _getDataDp = (id) => {
  ...
  $.ajax({ 
    "url"    : base_url+"PJ_Faktur_Penjualan/getdatauangmuka",       
    "type"   : "POST", 
    "dataType" : "json", 
    "data" : "id="+id.toString(),
    "cache"  : false,
    ...
    "error"  : function(xhr){
      console.log(xhr.responseText);
      alert('Error : Gagal mengambil data uang muka penjualan !');
      parent.window.$('.loader-wrap').addClass('d-none');                  
      return;
    },
    "success" : async function(result){
