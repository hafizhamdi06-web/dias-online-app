...
var _AmbilDetailPasien = () => { 
      ...
      $.ajax({ 
        ...
        { 
           ...
           {
            $('#divtglexpiredmember').append("<button type=\"button\" class=\"btn btn-danger btn-step1 text-sm btn-sm btn-block buttontambahan\" role=\"button\"> Member Tidak Aktif</button>");
            $('#statusmember').val(0);  
           }
           else if ( tglexpired < tglsekarang )  
           {
            $('#divtglexpiredmember').append("<button type=\"button\" class=\"btn btn-danger btn-step1 text-sm btn-sm  btn-block buttontambahan\" role=\"button\"> Member Sudah Expired Tanggal <span class=\"badge badge-light\" >"+result.data[0]['tglexpired']+"</span></button>");
            $('#statusmember').val(0);  
           }
           else 
           { 
            $('#divtglexpiredmember').append("<button type=\"button\" class=\"btn btn-light btn-step1 text-sm btn-sm  btn-block buttontambahan\" role=\"button\"> Member Aktif, Expired Tanggal <span class=\"badge badge-primary\" >"+result.data[0]['tglexpired']+"</span> </button>"); 
            $('#statusmember').val(1);  
              ...
              }  else  
              {
                  //_kontak = result.data[0]['kontak'] ; 
                        ...
                        if ( _jenis=='editharga')
                        {
                         ...
                         parent.window.toastr.success("Sukses membuka Kunci harga " + $("input[name^='item']").eq(_idx).val() );   
                        return; 
             }  
        }    
...
var _addRow = () => {
  let  
        ...
        newrow += "<span class=\"spannama col-4 col-form-label text-sm px-0 font-weight-normal\" id=\"spannama[]\" name=\"spannama[]\">Keterangan Produk</span>"; 
...
var _saveData = () => {
  ...
  const id = $("#id").val(),
        ...
        medid= $("#medid").val(),idmedlib=$("#idmedlib").val() ,teman= $("#teman").val()
