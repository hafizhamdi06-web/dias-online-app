...
var _cekPaket = (_NoPaket, _IdPaket) => {
  ...
  var _isOverduePOS = () => {
    ...
    var tgltrans = new Date(parts[2], parts[1]-1, parts[0]);
    var today = new Date();
    today.setHours(0,0,0,0);
    tgltrans.setHours(0,0,0,0);

    var selisihHari = (today - tgltrans) / (1000*60*60*24);
  ...
  var _cekPersetujuanPOS = () => {
    $.ajax({
      ...
      "type"   : "POST",
      "dataType" : "json",
      "data"   : "jenis=Edit POS Overdue&referensi="+encodeURIComponent($('#nomor').val()),
      "cache"  : false,
      "beforeSend" : function(){
        parent.window.$(".loader-wrap").removeClass("d-none");
      },
  ...
  var _tampilkanAjukanPersetujuanPOS = () => {
    $.ajax({
      ...
      "success" : function(list) {
        ...
        parent.window.Swal.fire({
          ...
          html:
            '<div class="text-left">'+
            '<label class="text-sm">Tanggal transaksi sudah lewat, pilih approver untuk mengajukan persetujuan:</label>'+
            '<select id="swal-approver-pos" class="form-control form-control-sm mb-2">'+options+'</select>'+
            '<label class="text-sm">Keterangan</label>'+
            '<input type="text" id="swal-keterangan-pos" class="form-control form-control-sm" value="EDIT POS No. '+$('#nomor').val()+'">'+
            '</div>',
          showCancelButton: true,
          ...
          preConfirm: () => {
            return {
              iduserapprover: parent.window.document.getElementById('swal-approver-pos').value,
              keterangan: parent.window.document.getElementById('swal-keterangan-pos').value
            };
          }
        }).then((result) => {
          if (result.isConfirmed) {
            _ajukanPersetujuanPOS(result.value.iduserapprover, result.value.keterangan);
          }
        });
      }
    });
  ...
  var _ajukanPersetujuanPOS = (iduserapprover, keterangan) => {
    $.ajax({
      ...
      "type"   : "POST",
      "dataType" : "json",
      "data"   : "jenis=Edit POS Overdue&referensi="+encodeURIComponent($('#nomor').val())+"&keterangan="+encodeURIComponent(keterangan)+"&iduserapprover="+iduserapprover,
      "cache"  : false,
      "beforeSend" : function(){
        parent.window.$(".loader-wrap").removeClass("d-none");
      },
    ...
    $.ajax({
      ...
      "success" : function(result) {
        ...
        if (result.status === 'disetujui') {
          ...
          $("input[name^='unlockedharga']").eq(idx).val('1');
  ...
  $("#btambahitem").click(function() {   
    ...
    var _CariKupon = (_JumlahKupon,_Jenis) => { 
    
    if($('#id').val()==''){
      $('#namakontak').attr('data-title','Transaksi belum disimpan!');
      $('#namakontak').tooltip('show');
      $('#namakontak').focus();
      return 0;
    }   
      ...
      $.ajax({ 
        ...
        "beforeSend": function(){
          parent.window.$(".loader-wrap").removeClass("d-none");
          parent.window.$(".modal").modal("show");                  
          parent.window.$(".modal-title").html("Cari Kupon " + _Jenis);
          parent.window.$("#modaltrigger").val("iframe-page-pos_2");
          parent.window.$('#coltrigger').val('pos');                
        },         
        "error": function(){
          parent.window.$(".loader-wrap").addClass("d-none");
          console.log('error menampilkan modal cari Kupon...');
          return;
        },
        "success": async function(result) {
          parent.window.$(".main-modal-body").html(result);
          ...
          parent.window._lstkategorikontak();
          parent.window._pilihkategorikontak(''); 
          parent.window.$('#idu').val($('#id').val());   
          parent.window.$('#sudahdapat').val('0'); 
          parent.window.$('#jumlahkupon').val(_JumlahKupon);  
          
           parent.window._kontakdatatable("view_table_voucher2");
    ...
    var _Cek_Kupon_OTC = (_Nomor, _Tanggal, _IDKontak, _Jumlahkupon ) => { 
        
        
         $.ajax({ 
        "url"    : base_url+"PJ_POS_HP/get_cek_kupon_otc", 
        "type"   : "POST", 
        //"data"   :  {notransaksi: $("#nomor").val(),tanggal:$("#tgl").val(),idkontak:$("#idkontak").val(),jumlahkupon:$("#jumlahkupon").val()    },
        "data"   :  {notransaksi: _Nomor,tanggal:_Tanggal,idkontak:_IDKontak,jumlahkupon:_Jumlahkupon    },
        "dataType" : "json", 
        "cache"  : false,
        "beforeSend" : function(){
          $("#loader-detil").removeClass('d-none');
        ...
        "success"  : async function(result) {  
          ...
          if(jumlah==0)
          { 
                 parent.window.toastr.error("Pasien tidak mendapatkan kupon OTC.");   
          }
          else
          {
             parent.window.toastr.success("Pasien mendapatkan kupon OTC.");   
                 _CariKupon(jumlah,'OTC');   
          } 
    ...
    var _Cek_Kupon_TINDAKAN = () => { 
       ...
       $.ajax({ 
        ...
        "error"  : function(xhr,status,error){ 
          parent.window.toastr.error("error cek kupon tindakan..");   
          $("#loader-detil").addClass('d-none');          
          return 0;
        },
        "success"  : async function(result) {  
          ...
          if(jumlah==0)
          { 
                 parent.window.toastr.error("Pasien tidak mendapatkan kupon TINDAKAN.");   
          }
          else
          {
             parent.window.toastr.success("Pasien mendapatkan kupon TINDAKAN.");    
                   await _CariKupon(jumlah,'Tindakan');   
          }
    ...
    function  _Cek_apakah_sudah_dapat_kupon() {  
     
       $.ajax({ 
        "url"    : base_url+"PJ_POS_HP/cek_apakah_sudah_dapat_kupon", 
        "type"   : "POST", 
        "data"   :  {idtransaksi: $("#id").val()},
        "dataType" : "json", 
        "cache"  : false,
        "beforeSend" : function(){
          $("#loader-detil").removeClass('d-none');
        ...
        "error"  : function(xhr,status,error){ 
          parent.window.toastr.error("error _Cek_apakah_sudah_dapat_kupon ..");   
          $("#loader-detil").addClass('d-none');          
          return 0;
        },
        "success"  : async function(result) { 
     ...
     var _Cek_Kupon_Sudah_Dapat = () => { 
        ...
        "success": function(result) {
          ...
          //parent.window._pilihkategorikontak(''); 
          parent.window.$('#idu').val($('#id').val());  
          parent.window.$('#jumlahkupon').val($('#jumlahkupon').val());   
          parent.window._kontakdatatable('view_table_voucher_sudahdapat');
           
          //parent.window._tampilkankupon(); 
  ...
  $("#bkupon").click(function() {   
   ...
   var  jumlahkuponsudahdapat = _Cek_apakah_sudah_dapat_kupon();
        alert(jumlahkuponsudahdapat);
   
    
    if( jumlahkuponsudahdapat >0 ){  
        
      
      _Cek_Kupon_Sudah_Dapat();
      return  ;
    }     
     
      _Cek_Kupon_OTC($("#nomor").val(),$("#tgl").val(),$("#idkontak").val(),$("#jumlahkupon").val());  
 ...
 $("#boknoic").click(function() {
  ...
  var _SetDataBarang = (_idx,_idbarang) => {
        ...
        $("input[name^='unlockedharga']").eq(_idx).val('0');
              ...
              }  else  
              {
                  //_kontak = result.data[0]['kontak'] ; 
                        ...
                        else if ( _jenis=='editdiskon')
                        {
                           ...
                           $("input[name^='unlockeddiskon']").eq(_idx).val('1');
                           $('#modalpassword').modal('hide');
...
var _addRow = () => {
  let  
        ...
        newrow += "<input name=\"unlockedharga[]\" type=\"hidden\" value=\"0\">";
        ...
        newrow += "<input name=\"unlockeddiskon[]\" type=\"hidden\" value=\"0\">";
...
var _saveData = () => {
  ...
  $("select[name^='item']").each(function(index,element){  
      detil.push({
               ...
               hargaunlocked:($("input[name^='unlockedharga']").eq(index).val()),
               diskonunlocked:($("input[name^='unlockeddiskon']").eq(index).val())
    ...
    "success": async function(result) {
      ...
      if(result.pesan=='sukses'){
       ...
       // let a =  CekKuponTech();
        ...
        //let jumlahkuponnya = await  _Cek_Kupon_OTC($("#nomor").val(),$("#tgl").val(),$("#idkontak").val(),$("#jumlahkupon").val());  
 ...
 $("#bkirimulangemail").click(function() {
    ...
    async function CekKuponTech() {

         
              try {
                const responsex = await _Cek_Kupon_OTC($("#nomor").val(),$("#tgl").val(),$("#idkontak").val(),$("#jumlahkupon").val());  
                
                return $("#jumlahkupon").val();
  ...
  $.ajax({ 
    ...
    "success" : function(result) {
      ...
      if (typeof result.pesan !== 'undefined') {
        ...
        $('#jumlahkupon').val(result.data[0]['jumlahkupon']);  
...
var _hitungJumlahDetil = (idx) => {   
  ...
  if($('#cabang').val()==18 && $("input[name^='unlockeddiskon']").eq(idx).val()!='1'){
    vdis1 = 0;
    vdis2 = 0;
