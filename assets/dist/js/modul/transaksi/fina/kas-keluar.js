...
/* ========================================================================================== */
  ...
  $("#bTable").click(() => {
      ...
      location.href=base_url+"page/bkkData";      
  });
  ...
  $("#bViewJurnal").click(() => {
      ...
      $.ajax({ 
          ...
          "beforeSend": () => {
              ...
              parent.window.$("#modaltrigger").val("iframe-page-bkk");
  ...
  $("#bsearch").click(() => {    
      $.ajax({ 
          ...
          "success": (result) => {        
              parent.window.$(".main-modal-body").html(result);  
              parent.window.$('.modal-body').css('min-height','calc(100vh - 30vh)');            
              parent.window._transaksidatatable('view_kas_keluar');
              setTimeout(function (){
                   parent.window.$('#modal input').focus();
              }, 500);
              return;
  ...
  $("#bprint").click(() => {
      if($('#id').val()=='') return;    
      window.open(`${base_url}Laporan/preview/page-bkk/${$("#id").val()}`)    
  });
  ...
  $(this).on("select2:select", "select[name^='coadebet']", function(){
      ...
      $.ajax({ 
          "url"    : base_url+"Fina_Kas_Keluar/get_coa", 
          "type"   : "POST", 
          "data"   : "id="+$(this).val(),
          "dataType" : "json", 
          "cache"  : false,
          ...
          "error"  : () => {
              console.log('error ambil informasi coa...');
...
var _deleteData = () => {
    ...
    $.ajax({ 
        "url"    : base_url+"Fina_Kas_Keluar/deletedata", 
        "type"   : "POST", 
...
var _saveData = () => {
    ...
    $.ajax({ 
        "url"    : base_url+"Fina_Kas_Keluar/savedata", 
        "type"   : "POST", 
        "data"   : rey,
        "processData": false,
        "contentType": false,
            ...
            if(result.pesan=='sukses'){
                parent.window.Swal.fire({
                    ...
                    if (printing.isConfirmed) {
                      window.open(`${base_url}Laporan/preview/page-bkk/${result.nomor}`)
                    }
...
var _tampilData = (result) => {
    if (typeof result.pesan !== 'undefined') {
        ...
        for (let i = 0; i < result.data.length; i++) _addRow();
...
var _getDataTransaksi = (id) => {
    ...
    $.ajax({ 
        "url"    : base_url+"Fina_Kas_Keluar/getdata",       
        "type"   : "POST", 
        "dataType" : "json", 
        "data" : "id="+id,
        "cache"  : false,
        ...
        "error"  : () => {
            parent.window.toastr.error('Error : Gagal mengambil data transaksi kas keluar !');
            parent.window.$('.loader-wrap').addClass('d-none');                  
            return;
        },
        "success" : (result) => {
