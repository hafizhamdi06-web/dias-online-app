...
$(function() {
  ...
  Component_Select2_Account(
    `#coakredit`,
    `${base_url}Select_Master/view_coa_bank`,
    `form_coa`,
    `Akun`
  );
  Component_Select2(
    `#bank`,
    `${base_url}Select_Master/view_bank`,
    `form_bank`,
    `Bank`
  );
...
/* ========================================================================================== */
  ...
  $("#bTable").click(() => {
      ...
      location.href=base_url+"page/bbkData";      
  });
  ...
  $("#tipebayar").on("select2:select",function() {
    if($(this).val() == 1) {
      ...
      $("#colclear").removeClass('col-sm-6');
      $("#colclear").addClass('col-sm-2');            
      $("#bank").val('').change();
      return;
    }
    if($(this).val() == 2) {
      ...
      $("#colclear").addClass('col-sm-2');
      $("#nomorgiro").val('');      
      $("#tglgiro").datepicker('setDate','');            
      return;
    }
    if($(this).val() == 0) {
      $(".giro").addClass('d-none');
      $(".transfer").addClass('d-none');      
      $("#colclear").removeClass('col-sm-2');
      $("#colclear").addClass('col-sm-6');            
      $("#bank").val('').change();
      $("#nomorgiro").val('');      
      $("#tglgiro").datepicker('setDate','');            
      return;
  ...
  $("#bsearch").click(() => {    
      $.ajax({ 
          ...
          "success": (result) => {
              parent.window.$(".main-modal-body").html(result);     
              parent.window.$('.modal-body').css('min-height','calc(100vh - 30vh)');         
              parent.window._transaksidatatable('view_bank_keluar');
              setTimeout(function (){
                   parent.window.$('#modal input').focus();
              }, 500);
              return;
  ...
  $("#baddrow").click(() => {
      _addRow();
      _inputFormat();
      $("select[name^='coadebet']").last().focus();        
  });
  ...
  $("#bprint").click(() => {
      if($('#id').val()=='') return;    
      window.open(`${base_url}Laporan/preview/page-bbk/${$("#id").val()}`)    
  });
  ...
  $(this).on("select2:select", "select[name^='coadebet']", function(){

      if($(this).val()=="" || $(this).val()==null) return;

      let _idx = $(this).index('.coadebet');

      $.ajax({ 
          "url"    : base_url+"Fina_Bank_Keluar/get_coa", 
          "type"   : "POST", 
          "data"   : "id="+$(this).val(),
          "dataType" : "json", 
          "cache"  : false,
          ...
          "error"  : () => {
              console.error('error ambil informasi coa...');
...
/* ========================================================================================== */
  ...
  var _addRow = () => {
      let newrow = " <tr>";
          newrow += "<td><select name=\"coadebet[]\" class=\"coadebet form-control select2 form-control-sm\" style=\"width:100%;\" data-trigger=\"manual\" data-placement=\"auto\"></select></td>";
          newrow += "<td><input type=\"hidden\" name=\"idmuangdebet[]\" class=\"idmuangdebet\"><div class=\"input-group\"><div class=\"input-group-append\"><div class=\"muangdebet input-group-text bg-white border-right-0 py-0 px-2\">Rp</div></div><input type=\"tel\" name=\"amtdebet[]\" class=\"amtdebet form-control form-control-sm numeric\" autocomplete=\"off\" value=\"0\"></div></td>";
...
var _IsValid = (() => {
    ...
    if ($('#coakredit').val()=='' || $('#coakredit').val()==null){
      $('#coakredit').attr('data-title','Akun yang di kredit harus diisi !');      
      $('#coakredit').tooltip('show');
      $('#coakredit').focus();
      return 0;
    }

    let totalbaris = $(".coadebet").length;
    for(let i=0;i<totalbaris;i++){
      if($("select[name^='coadebet']").eq(i).val()=='' || $("select[name^='coadebet']").eq(i).val()==null){
          $("select[name^='coadebet']").eq(i).attr('data-title','Akun harus diisi !');      
          $("select[name^='coadebet']").eq(i).tooltip('show');      
          $("select[name^='coadebet']").eq(i).focus();
          return 0;
      }
    }
...
var _saveData = (() => {
  ...
  let id = $("#id").val(),
      ...
      uraian = $("#uraian").val(),
      coacr = $("#coakredit").val(),
      uangcr = $("#iduangkredit").val(),
      kurscr = Number($("#kurskredit").val().split('.').join('').toString().replace(',','.')),
  ...
  $("select[name^='coadebet']").each(function(index,element){  
      detil.push({
               coa:this.value,
               jml:Number($("input[name^='amtdebet']").eq(index).val().split('.').join('').toString().replace(',','.')),
  ...
  rey.set('coacr',coacr);
  rey.set('uangcr',uangcr);
  rey.set('kurscr',kurscr);
  ...
  $.ajax({ 
      "url"    : base_url+"Fina_Bank_Keluar/savedata", 
      "type"   : "POST", 
      "data"   : rey,
      "processData": false,
      "contentType": false,
            ...
            if(result.pesan=='sukses'){
                parent.window.Swal.fire({
                    ...
                    if (printing.isConfirmed) {
                      window.open(`${base_url}Laporan/preview/page-bbk/${result.nomor}`)
                    }
...
var _tampilData = (result) => {
      if (typeof result.pesan !== 'undefined') {
        ...
        var coaCr = $("<option selected='selected'></option>").val(result.data[0]['coacrid']).text(result.data[0]['coacr']);            
        var _bank = $("<option selected='selected'></option>").val(result.data[0]['idbank']).text(result.data[0]['bank']);
        ...
        $('#iduangkredit').val(result.data[0]['iduangkredit']);                        
        $('#uangkredit').val(result.data[0]['uangkredit']);                        
        $('#kurskredit').val(result.data[0]['kurskredit'].replace(".", ","));                        
        $('#coakredit').append(coaCr).trigger('change');
        ...
        if(result.data[0]['bank'] != null && result.data[0]['idbank'] != 0) $('#bank').append(_bank).trigger('change');          
        if(result.data[0]['tglgiro'] != null) $('#tglgiro').datepicker('setDate',result.data[0]['tglgiro']);          
        $('#nomorgiro').val(result.data[0]['giro']);                        

        var rows = 0;
        $.each(result.data, function() {
            var coaDb = $("<option selected='selected'></option>").val(result.data[rows]['coadetilid']).text(result.data[rows]['coadetil']);            
            var divisi = $("<option selected='selected'></option>").val(result.data[rows]['divisidetilid']).text(result.data[rows]['divisidetil']);            

            $("select[name^='coadebet']").eq(rows).append(coaDb).trigger('change');            
            $("input[name^='amtdebet']").eq(rows).val(result.data[rows]['jmldetil'].replace(".", ","));
            ...
            if(result.data[rows]['jmldetil']==0) $("input[name^='amtdebet']").eq(rows).attr('placeholder','0,00');            
...
var _getDataTransaksi = (id) => {
    ...
    $.ajax({ 
        "url"    : base_url+"Fina_Bank_Keluar/getdata",       
        "type"   : "POST", 
        "dataType" : "json", 
        "data" : "id="+id,
        "cache"  : false,
        ...
        "error"  : () => {
            parent.window.toastr.error('Error : Gagal mengambil data transaksi bank keluar !');
            parent.window.$('.loader-wrap').addClass('d-none');                  
            return;
        },
        "success" : (result) => {
...
var _hitungsubtotal = () => {
  ...
  $('.coadebet').each(function(index,element) {
      trupiah += Number($("input[name^='amtdebet']").eq(index).val().split('.').join('').toString().replace(',','.')); 
