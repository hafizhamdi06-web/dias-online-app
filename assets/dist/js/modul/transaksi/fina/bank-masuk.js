...
/* ========================================================================================== */
  ...
  $("#bTable").click(() => {
      ...
      location.href=base_url+"page/bbmData";      
  });
  ...
  $("#bViewJurnal").click(() => {
      ...
      $.ajax({ 
          ...
          "beforeSend": () => {
              ...
              parent.window.$("#modaltrigger").val("iframe-page-bbm");
  ...
  $("#bsearch").click(() => {    
      $.ajax({
          ...
          "success": (result) => {        
              parent.window.$(".main-modal-body").html(result);   
              parent.window.$('.modal-body').css('min-height','calc(100vh - 30vh)');           
              parent.window._transaksidatatable('view_bank_masuk');
              setTimeout(function (){
                   parent.window.$('#modal input').focus();
              }, 500);
              return;
  ...
  $("#bprint").click(() => {
      if($('#id').val()=='') return;    
      window.open(`${base_url}Laporan/preview/page-bbm/${$("#id").val()}`)    
  });
  ...
  $(this).on("select2:select", "select[name^='coakredit']", function(){
      ...
      $.ajax({ 
          "url"    : base_url+"Fina_Bank_Masuk/get_coa", 
          "type"   : "POST", 
          "data"   : "id="+$(this).val(),
          "dataType" : "json", 
          "cache"  : false,
...
var _deleteData = () => {
    ...
    $.ajax({ 
        "url"    : base_url+"Fina_Bank_Masuk/deletedata", 
        "type"   : "POST", 
...
var _saveData = () => {
    ...
    $.ajax({ 
        "url"    : base_url+"Fina_Bank_Masuk/savedata", 
        "type"   : "POST", 
        "data"   : rey,
        "processData": false,
        "contentType": false,
            ...
            if(result.pesan=='sukses'){
                parent.window.Swal.fire({
                    ...
                    if (printing.isConfirmed) {
                      window.open(`${base_url}Laporan/preview/page-bbm/${result.nomor}`)
                    }
...
var _getDataTransaksi = (id) => {
    ...
    $.ajax({ 
        "url"    : base_url+"Fina_Bank_Masuk/getdata",       
        "type"   : "POST", 
        "dataType" : "json", 
        "data" : "id="+id,
        "cache"  : false,
        ...
        "error"  : () => {
            parent.window.toastr.error('Error : Gagal mengambil data transaksi bank masuk !');
            parent.window.$('.loader-wrap').addClass('d-none');                  
            return;
        },
        "success" : (result) => {
